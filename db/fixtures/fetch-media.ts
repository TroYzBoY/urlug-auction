...
async function main() {
  ...
  /*
   * The same facts again, in a form the site can render. CREDITS.md is for a
   * person reading the repository; this is for the gallery, which has to show
   * the attribution beside the image for a CC BY or CC BY-SA file to be used
   * lawfully at all. A licence condition only a maintainer can see is not being
   * met.
   */
  const generated = [
    "/**",
    ...
    ...credits.map(
      (c) =>
        `  ${JSON.stringify("/media/lots/" + c.file)}: ` +
        `${JSON.stringify(`Зураг: ${c.author} (${c.licence})`)},`,
    ),
    "};",
    "",
  ];
  await writeFile(join(here, "credits.ts"), generated.join("\n"), "utf8");
