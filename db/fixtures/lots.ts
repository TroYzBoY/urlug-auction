 ...
 * ── On the photographs ───────────────────────────────────────────────────────
 *
 * ⚠ The image paths below are PLACEHOLDERS generated by
 * `db/fixtures/make-placeholders.ts`. They are labelled SVGs, not product
 * photography, and they are deliberately obvious about it — a placeholder that
 * looks like a real photo is one nobody remembers to replace.
 *
 * Real listings need real photographs of the actual unit being sold, from the
 * angles named in each `alt` below. That is not a nicety: a bidder committing
 * to a used phone sight unseen is entitled to see its back, its ports and its
 * screen, and a stock render of a different unit is closer to a
 * misrepresentation than to a catalogue.
 *
 * ── On the descriptions ──────────────────────────────────────────────────────
 *
...
/** Four angles is the minimum a used device can be judged from. */
function views(id: string, labels: string[]) {
  return labels.map((alt, i) => ({
    url: `/media/lots/${id}/${String(i + 1).padStart(2, "0")}.svg`,
    alt,
  }));
}
...
iCloud-оос гарсан, Activation Lock идэвхгүй. IMEI цэвэр, аль ч оператор дээр ажиллана.`,
    provenance: "Улаанбаатар, нэг эзэнтэй байсан, 2026 оны 3-р сард худалдаж авсан",
    ...
    openingPts: 2400,
    images: views("101", [
      "Урд тал — дэлгэц асаалттай",
      "Ар тал — камерын блок",
      "Хажуу тал — титан хүрээ, товчлуурууд",
      "Ар талын доод булан — 3мм маажилт",
      "Иж бүрдэл — хайрцаг, кабель",
    ]),
    status: "live",
...
macOS сүүлийн хувилбар суусан, Find My унтраасан, дискийг бүрэн цэвэрлэсэн.`,
    ...
    openingPts: 4200,
    images: views("102", [
      "Нээлттэй — дэлгэц ба гар",
      "Хаалттай — тагны лого тал",
      "Тагны гадаргуу — нимгэн зураас",
      "Зүүн тал — Thunderbolt, MagSafe",
      "Баруун тал — HDMI, SDXC",
    ]),
    status: "live",
...
Цуврал дугаар шалгагдсан, Apple-ийн баталгаа 2027 оны 2-р сар хүртэл хүчинтэй.`,
    ...
    images: views("103", [
      "Кейс хаалттай",
      "Кейс нээлттэй — чихэвчүүд",
      "Чихэвч ойроос — мэдрэгч",
      "Сеткэн тор — цэвэр байдал",
    ]),
    status: "live",
...
Данснаас гарсан, дискийг бүрэн форматласан. Бүх шинэчлэлт суусан.`,
    ...
    openingPts: 1800,
    images: views("104", [
      "Урд тал — босоо байрлал",
      "Ар тал — портууд",
      "Дээд бүрхүүл",
      "DualSense Edge удирдлага",
      "Иж бүрдэл — кабель, тавиур",
    ]),
    status: "upcoming",
...
eSIM идэвхжүүлээгүй, оператороос салгагдсан.`,
    ...
    images: views("105", [
      "Урд тал — дэлгэц",
      "Ар тал — камер",
      "Apple Pencil Pro-тэй",
      "Magic Keyboard-тэй",
      "Ар талын алюмин — хэрэглээний шугам",
    ]),
    status: "upcoming",
...
Байдал: Титан биеийн зүүн ирмэгт микро түвшний ул мөр (3 дахь зурагт). Дижитал титэм, Action товч бүгд хэвийн.`,
    ...
    images: views("106", [
      "Урд тал — дэлгэц асаалттай",
      "Ар тал — мэдрэгчүүд",
      "Зүүн ирмэг — микро ул мөр",
      "Хоёр оосор — Trail болон Ocean",
    ]),
    status: "upcoming",
...
iCloud-оос гарсан, IMEI цэвэр.`,
    ...
    images: views("109", [
      "Урд тал",
      "Ар тал",
      "Хажуу тал — Камерын товч",
      "Баруун доод булан — унасны мөр",
    ]),
    status: "upcoming",
