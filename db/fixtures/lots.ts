/**
 * Sample catalogue — 12 lots across all six categories.
 *
 * These were `src/lib/mock.ts`. Moving them under `db/` is the point of the
 * change, not a tidy-up: nothing in `src/` can import this file, so there is no
 * longer any path by which mock data reaches a running server. They are seed
 * fixtures now, loaded by db/seed.ts, and the `status` on each one only decides
 * where in the schedule the seeder places it.
 */
import type { Lot } from "../../src/lib/types.ts";

/*
 * One live, eight upcoming, two sold, one unsold — every status the UI can
 * render, so a seeded database exercises all of them.
 */
